...
#define TELEPORT_GRENADE_BLIP_FREQUENCY			1.0f
#define TELEPORT_GRENADE_BLIP_FAST_FREQUENCY	0.3f

#define TELEPORT_GRENADE_GRACE_TIME_AFTER_PICKUP 1.5f
#define TELEPORT_GRENADE_WARN_TIME 1.5f

const float TELEPORT_GRENADE_COEFFICIENT_OF_RESTITUTION = 0.2f;
...
BEGIN_DATADESC( CGrenadeTeleport )
...
// Purpose: 
//-----------------------------------------------------------------------------
CGrenadeTeleport::~CGrenadeTeleport( void )
{
}

void CGrenadeTeleport::Spawn( void )
{
    Precache();
    ...
    {
        BlipSound();
        m_flNextBlipTime = gpGlobals->curtime + TELEPORT_GRENADE_BLIP_FREQUENCY;
    }
#else
    BlipSound();
    m_flNextBlipTime = gpGlobals->curtime + TELEPORT_GRENADE_BLIP_FREQUENCY;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeTeleport::OnRestore( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CGrenadeTeleport::CreateEffects( void )
{
    ...
    if ( m_pGlowTrail != NULL )
    {
        ...
        m_pGlowTrail->SetTransparency( kRenderTransAdd, 0, 160, 110, 255, kRenderFxNone );
...
bool CGrenadeTeleport::CreateVPhysics()
{
...
class CTraceFilterCollisionGroupDeltaTeleport : public CTraceFilterEntitiesOnly
{
    public:
    // It does have a base, but we'll never network anything below here..
    DECLARE_CLASS_NOBASE( CTraceFilterCollisionGroupDeltaTeleport );
    CTraceFilterCollisionGroupDeltaTeleport( const IHandleEntity* passentity, int collisionGroupAlreadyChecked, int newCollisionGroup )
...
void CGrenadeTeleport::VPhysicsUpdate( IPhysicsObject* pPhysics )
{
    ...
    CTraceFilterCollisionGroupDeltaTeleport filter( this, GetCollisionGroup(), COLLISION_GROUP_NONE );
    trace_t tr;
    ...
    if ( tr.startsolid )
    {
        if ( !m_inSolid )
        {
            ...
            vel *= -TELEPORT_GRENADE_COEFFICIENT_OF_RESTITUTION; // bounce backwards
    ...
    if ( tr.DidHit() )
    {
        ...
        // absorb 80% in impact
        vel *= TELEPORT_GRENADE_COEFFICIENT_OF_RESTITUTION;
        angVel *= -0.5f;
...
void CGrenadeTeleport::Precache( void )
{
...
void CGrenadeTeleport::SetTimer( float detonateDelay, float warnDelay )
{
    m_flDetonateTime = gpGlobals->curtime + detonateDelay;
    m_flWarnAITime = gpGlobals->curtime + warnDelay;
    SetThink( &CGrenadeTeleport::DelayThink );
    SetNextThink( gpGlobals->curtime );

    CreateEffects();
}

void CGrenadeTeleport::OnPhysGunPickup( CBasePlayer* pPhysGunUser, PhysGunPickup_t reason )
{
    SetThrower( pPhysGunUser );

#ifdef HL2MP
    SetTimer( TELEPORT_GRENADE_GRACE_TIME_AFTER_PICKUP, TELEPORT_GRENADE_GRACE_TIME_AFTER_PICKUP / 2 );

    BlipSound();
    m_flNextBlipTime = gpGlobals->curtime + TELEPORT_GRENADE_BLIP_FAST_FREQUENCY;
    m_bHasWarnedAI = true;
#else
    if ( IsX360() )
    {
...
void CGrenadeTeleport::DelayThink()
{
...
void CGrenadeTeleport::SetVelocity( const Vector& velocity, const AngularImpulse& angVelocity )
{
...
int CGrenadeTeleport::OnTakeDamage( const CTakeDamageInfo& inputInfo )
{
...
bool CGrenadeTeleport::HandleInteraction( int interactionType, void* data, CBaseCombatCharacter* sourceEnt )
{
    ...
    if ( interactionType == g_interactionBarnacleVictimGrab )
    {
        ...
        SetTimer( timer, timer - TELEPORT_GRENADE_WARN_TIME );
...
void CGrenadeTeleport::InputSetTimer( inputdata_t& inputdata )
{
...
CBaseGrenade* Teleportgrenade_Create( const Vector& position, const QAngle& angles, const Vector& velocity, const AngularImpulse& angVelocity, CBaseEntity* pOwner, float timer, bool combineSpawned )
{
    ...
    pGrenade->SetTimer( timer, timer - TELEPORT_GRENADE_WARN_TIME );
